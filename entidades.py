...
Por ahora solo se implementa lo necesario para esta primera entrega:
- EntidadJuego (abstracta)
- Kael (jugador), con movimiento horizontal y salto.
"""
...
class EntidadJuego(ABC):
    ...
    def __init__(self, x: float, y: float, ancho: int, alto: int, ruta_imagen: str = "", color_placeholder=(255, 0, 255)):
        ...
        self.imagen = self._cargar_imagen(ruta_imagen, ancho, alto, color_placeholder)
        self.rect = self.imagen.get_rect(topleft=(self.x, self.y))

    @staticmethod
    def _cargar_imagen(ruta_imagen: str, ancho: int, alto: int, color_placeholder):
        """
...
class Kael(EntidadJuego):
    ...
    def manejar_entrada(self, teclas):
        """Lee el estado del teclado y define la velocidad horizontal y el salto."""
        self.velocidad_x = 0
    ...
    def actualizar(self, suelo_y: int):
        """
        Mueve a Kael segun su velocidad actual, aplica gravedad y resuelve
        la colision simple con el suelo.
